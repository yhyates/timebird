...
private func addnotification(whooo: String, releaseDateee: Date) {
    ...
    if (timefar(day: releaseDateee)>0) {
        ...
        if timefar(day: releaseDateee) > 7 {
            print("大于7天")
            noti7(date: releaseDateee, Islunar: "公历")
        ...
        } else if (3 <= timefar(day: releaseDateee)) && (timefar(day: releaseDateee) < 7) {
            noti3(date: releaseDateee, Islunar: "公历")
            noti1(date: releaseDateee, Islunar: "公历")
            noti(year: intyear, month: intmonth, day: intday, during: 0, islunar: "公历", whoo: whooo)
        ...
        ScrollView(showsIndicators: false) {
            ...
            VStack(alignment: .leading){
                Text("1⃣️【时间百分比】精度到小时，自动更新。")
                    ...
                    .padding(.bottom)
                Text("2⃣️【循环通知】可用作番茄钟，感知时间流和定时提醒等功能，时间单位为分钟；记录近七天数据；本功能无需常驻后台。")
                    .font(.footnote)
                    .foregroundColor(Color.gray)
                    .padding(.bottom)
                Text("3⃣️【记录时间点】一般记录如生日，纪念日，待办等节点；长按拖动可调整顺序，左滑可删除；支持农历，利用时间选择器输入阳历数据，将自动计算对应本年的农历日期，并显示农历公历倒计时；自动注册动态通知，包括前7天，前3天，前天以及当天提醒，通知具体时间为上午8:30-9:30。")
                    .font(.footnote)
                    .foregroundColor(Color.gray)
                    .padding(.bottom)
            }
        }
        .navigationBarTitle("🧐Time秘籍")
