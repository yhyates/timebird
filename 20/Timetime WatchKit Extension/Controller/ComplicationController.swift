...
import SwiftUI
import LunarCore
var ratioyear: String {
    String(format: "%.3f", yearbug(year: yearfresh(), day: yeardayfresh(), hour: hourfresh()))
}
var ratiomonth: String {
    String(format: "%.3f", monthbug(year: yearfresh(), month: monthfresh(), day: monthdayfresh(), hour: hourfresh()))
}
var ratioweek: String {
    String(format: "%.3f", weekbug(day: weekfresh(), hour: hourfresh()))
}
var lunarday: String {
    "\(LunarCore.solarToLunar(yearfresh(), monthfresh(), monthdayfresh())["lunarDayName"]!)" as String
}
var lunaryear: String {
    "\(LunarCore.solarToLunar(yearfresh(), monthfresh(), monthdayfresh())["GanZhiYear"]!)" as String
}
let lunar_12pic = [
    1:"🐮",
    2:"🐮",
...
var lunarpic: String {
    lunar_12pic[Calendar.current.component(.hour, from: Date())]!
}
var lunarmoonpic: String {
    lunar_moonpic[lunarday]!
}
var lunarzodic: String {
    "\(LunarCore.solarToLunar(yearfresh(), monthfresh(), monthdayfresh())["zodiac"]!)" as String
}
var lunarpic2: String {
    lunar_12pic2[lunarzodic]!
}
var originlunar: String {
    "\(lunar())"
}
let startIndex =  originlunar.index(originlunar.endIndex, offsetBy: -2)
var hourlunar: String {
    String(originlunar[startIndex..<originlunar.endIndex])
}
        ...
        VStack{
            ...
            ZStack{
                Text("今日循环时长：\(todaydata)分钟")
                    .font(.system(size:13))
                    .foregroundColor(Color("cutegray"))
...
class ComplicationController: NSObject, CLKComplicationDataSource {
    func getComplicationDescriptors(handler: @escaping ([CLKComplicationDescriptor]) -> Void) {
        ...
        let descriptors = [
            CLKComplicationDescriptor(identifier: "complication", displayName: "时鸟·Timebird", supportedFamilies: families)
        ]
        handler(descriptors)
    }
