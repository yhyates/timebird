...
public struct ChartForm {
    ...
    public static let large = CGSize(width:WKInterfaceDevice.current().screenBounds.size.width, height:90)//180
            ...
            VStack(alignment: .leading){
                HStack{
                    if(!showValue){
                        ...
                        if (self.currentValue) <= 60.0 {
                            Text("\(Int(self.currentValue))分钟")
                                .font(.footnote)
                                .foregroundColor(self.colorScheme == .dark ? self.darkModeStyle.textColor : self.style.textColor)
                        } else {
                            let currentValuehour = Int(self.currentValue)/60
                            let currentValuemin = Int(self.currentValue)%60
                            Text("\(currentValuehour)小时\(currentValuemin)分钟")
                                .font(.footnote)
                                .foregroundColor(self.colorScheme == .dark ? self.darkModeStyle.textColor : self.style.textColor)
                        }
