    ...
    var body: some View {
        ...
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading){
                Text("0⃣️【年龄记录】输入 阳历 年月日数据，自动计算来到世界多少天，选择自己或者更重要的人，记录她的年华")
                    .font(.footnote)
                ...
                Text("3⃣️【Todo清单】支持计数，长按✔︎可累加计数，点按则完成本任务。")
                    ...
                    .foregroundColor(Color.gray)
                    .padding(.bottom)
                Text("4⃣️【记录时间点】一般记录如生日，纪念日，待办等节点；长按拖动可调整顺序，左滑可删除；支持农历，利用时间选择器输入 阳历 数据，将自动计算对应本年的农历日期，并显示农历公历倒计时；自动注册动态通知提醒，包括前3天，前天以及当天提醒，通知具体时间为上午8:30-9:30。")
                    .font(.footnote)
                    .foregroundColor(Color.gray)
                    .padding(.bottom)
                Text("5⃣️【复杂功能】后台每小时安排一次刷新；从表盘点击时鸟复杂功能会自动刷新表盘，但仍限制每小时刷新一次")
                    .font(.footnote)
                    .foregroundColor(Color.gray)
                    .padding(.bottom)
            }
