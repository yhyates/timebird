...
func noti(year: Int, month: Int, day: Int, during: Int, islunar: String, whoo: String) {
    if during == 7{
        ...
        content.title = "事件：\(whoo)"
        content.body = "将7天后到达[\(islunar)]计算节点，请及时做好安排"
        ...
        let content = UNMutableNotificationContent()
        content.title = "事件：\(whoo)"
        content.body = "将3天后到达[\(islunar)]计算节点，请及时做好安排"
        content.sound = UNNotificationSound.defaultCritical
        var dateComponents = DateComponents()
        dateComponents.year = year
        dateComponents.month = month
        ...
        let content = UNMutableNotificationContent()
        content.title = "事件：\(whoo)"
        content.body = "将明天到达[\(islunar)]计算节点，别忘记了"
        content.sound = UNNotificationSound.defaultCritical
        var dateComponents = DateComponents()
        dateComponents.year = year
        dateComponents.month = month
        ...
        let content = UNMutableNotificationContent()
        content.title = "事件：\(whoo)"
        content.body = "今天已到[\(islunar)]计算节点！"
        content.sound = UNNotificationSound.defaultCritical
        var dateComponents = DateComponents()
        dateComponents.year = year
        dateComponents.month = month
...
struct AddTime: View {
    ...
    @State private var selectedYear = (0 <= (yearfresh()-1900) && (yearfresh()-1950) <= 135) ? yearfresh()-1900 : 71
...
struct inf: View {
    ...
    var body: some View {
        ...
        ScrollView(showsIndicators: false) {
            Text("事件：\(movie.who!)")
                .lineLimit(1)
            Text("\(String(dateyear2))年\(solormonth)月\(solorday)日")
                .lineLimit(1)
                .font(.footnote)
                .padding(.bottom)
